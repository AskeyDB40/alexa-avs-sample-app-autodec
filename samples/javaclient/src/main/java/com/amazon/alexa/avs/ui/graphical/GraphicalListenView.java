...
import com.amazon.alexa.avs.SpeechRequestAudioPlayerPauseController;

public class GraphicalListenView extends JPanel implements ListenUIHandler {
	private static final Logger log = LoggerFactory.getLogger(GraphicalListenView.class);
	...
	//wendy add
	private AutoDecThread thread;
	private SpeechRequestAudioPlayerPauseController autodec;
    ...
    GraphicalListenView(RecordingRMSListener rmsListener, AVSController controller) {
		...
		//wendy add
		autodec.autodec = true;
		thread = new AutoDecThread();
        thread.start();
		//onWakeWordDetected();
    }
    ...
    public void onProcessingFinished() {
		log.info("Wendy: GraphicalListenView --- onProcessingFinished ---- start");
		...
		log.info("Wendy: GraphicalListenView --- onProcessingFinished ---- end");
    }
    ...
    public synchronized void onAccessTokenReceived(String accessToken) {
		log.info("Wendy: GraphicalListenView --- onAccessTokenReceived ---- start");
	...
	//wendy add
	private class AutoDecThread extends Thread {
		@Override
		public void run() {
			while (true)
			{
				if(autodec.autodec)
				{
					log.info("Wendy: GraphicalListenView --- AutoDecThread");
					onWakeWordDetected();
					autodec.autodec = false;
				}
